...
and the noise covariance matrix $N = \ev{\vbn \vbn^{\dagger}}$ is diagonal in \textgreen{the} frequency space.
...
The noise covariance matrix $N$ is often sparse in \textgreen{the} frequency domain and the pointing matrix $P$ is sparse in the time-by-pixel domain.
In experiments currently under design, there may be $\sim 10^{16}$ time samples and $\sim 10^{9}$ pixels, so these matrix inversions are intractable unless the covariance is uniform \textgreen{(proportional to the identity matrix $I$)}.
...
\textgreen{
In our numerical experiments, we confirm the conclusion from \citet{2018A&A...620A..59P} that the conjugate gradient approach is converging faster than the fixed point iterations suggested by the messenger map-making method in \citet{Huffenberger_2018}.
}
...
because $N(\eta) = \lambda^{-1} N(\lambda)$ and the condition number does not change upon  scalar multiples of both sides of the equation.
}
...
Since $N$ is diagonal in \textgreen{the} frequency space,
...
\strikeThrough{
The solver may get stuck at \kmh{some local minimum {point}} in $\chi^2$ and have trouble getting to the global minimum.
}
\kmh{(Question: if $\chi^2$ is a quadratic form, are there any local minima other than the global one?)}
\textgreen{
(Good point, I think this part is an error, and I'm not sure why vanilla CG would be slow,.)
}
...
$P\vbm$ from \textgreen{the} time domain to frequency domain and an inverse Fourier transform
of $\inv{N(\eta_i)} P \vbm$ from \textgreen{the} frequency domain back to time domain,
...
The sky signal  in the timestream has \strikeThrough{an rms} \textgreen{a root-mean-square (RMS)} of $56$ $\mu$K.
...
\end{equation}
is a diagonal matrix in \textgreen{the} frequency space, where $\Delta_f$ is equal to {the} reciprocal
...
For this work, \textgreen{\strikeThrough{BQQ} QBQ (just feel BQQ sounds like BBQ)}  and KMH are supported by NSF award 1815887.
\end{acknowledgments}
...
\textgreen{
Since we can't calculate $\vbd - P\hatm(\eta_m)$, 
}
...
can be simultaneously diagonalized in \textgreen{the} frequency space.
