...
We choose the $\eta_i$ from a geometric series
\begin{align}
...
So we need to find a way to improve Eq. \ref{etai rule}, such that it increase faster.
...
low frequency noise by using Eq.~(\ref{exact eta}) the result is close to
...
then $\qty|\delta \chi^2(\hatm(0),0)| \simeq \chi^2(\hatm(0),0)$.
We would expect
\begin{align}
-\frac{\delta\chi^2(\hatm(0),0)}{\chi^2(\hatm(0),0)}
