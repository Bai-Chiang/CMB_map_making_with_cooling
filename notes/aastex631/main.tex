...
\end{align}
where  $\hatm$ provides the generalized least squares minimization of the $\chi^2$ statistic,\textcolor{green}{ which for an arbitrary map vector $\vbm$ is }
...
From the starting point, the cooling scheme requires the inverse cooling parameter $\eta$ increase as $0 = \eta_0 \leq \eta_1 \leq \cdots \leq \eta_{\rm final} = 1$, at which point we arrive at the desired mapmaking equation,
    and $\hatm = \hatm(1)$.
...
and move to next parameter $\eta_{i+1}$ when the residual \textcolor{red}{
 is smaller than a tenth of the pixel variance given by the white noise level of the noise model,
\begin{align}\qty(\Pdagger \inv{N(\eta_i)} P)\, 
\hatm(\eta_i) - \Pdagger \inv{N(\eta_i)} \vbd < 0.1 {\sigma^2_{\rm pix}}. \end{align}
...
equations $A(\eta_i) \vbm = \vbb(\eta_i)$ with
