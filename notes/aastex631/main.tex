...
The solver may get stuck at \kmh{some local minimum {point}} in $\chi^2$ and have trouble getting to the global minimum. \kmh{(Question: if $\chi^2$ is a quadratic form, are there any local minima other than the global one?)}
...
\centering
\caption{Noise power spectra that we use in our map-making simulations.  These show a variety of low-frequency behavior, parameterized by \Eq{noise power spectrum}, with \kmh{white noise at high frequency 
    \sout{white-noise level $\sigma^2 = 10$ $\mu$K$^2$} and a} low-frequency power-law slope $\alpha = 3$.
...
In numerical simulations, we find that if we update the $\eta$ parameter based on \kmh{the more precise} \Eq{delta eta update},
