...
In Section \ref{sec:discussion} we intuitively interpret our map-making approach, and point out possible improvement direction.
...
$\Nbar \equiv N - \tau I$ is the non-uniform part of the original noise covariance $N$.
...
\begin{equation}
  N(\eta) = \tau I +  \eta \Nbar 
\end{equation}
which leads to the same system of map-making equations.  (This is because $N(\eta) = \lambda^{-1} N(\lambda)$ and the map-making equation (\ref{map making eq eta}) is insensitive to to scalar multiple of the covariance since is appears on both sides.)
This parameterized covariance $N(\eta)$ equals the original covariance $N$ when $\eta = 1$.
