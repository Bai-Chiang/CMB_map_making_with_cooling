...
\end{align}
as we adjust the parameter. 
\textcolor{green}{
    Note that the estimated map $\hatm(\eta)$ is depends on $\eta$.
    In this paper if we write $\hatm$ without $\eta$ argument,
    that means the $\hatm$ in the Eq.~(\ref{map making eq}),
    which is not depends on $\eta$.
}
