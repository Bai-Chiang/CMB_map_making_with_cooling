...
and equation (\ref{map making eq eta}) could be simplified as $A(\eta_i) \hatm(\eta_i) = b(\eta_i)$.
...
which means the residual $\vb{r}(\vbm, \eta_{i+1})$ need to be calculated basd on new $\eta_{i+1}$
and then starts conjugate gradient iterations.
This residual calculation contains an extra $A(\eta_i) \vbm$ operation.
Therefore, overall if we have a series $\eta_1, \eta_2, \eta_3, \cdots, \eta_{n_{\eta}}$,
...
To have a fair comparision between parameterized and traditional conjugate gradient method,
...
it ends up with some less $\eta$ parameters, but the inverval between $\eta_i$ and $\eta_{i+1}$ gets larger. 
