...
%\textcolor{green}{
%    From now on we will use $\hatm$ donote an estimated map $\hatm = \PPinv{\inv{N}} \Pdagger \inv{N} \vbd$,
%    and $\vbm$ for arbitary map size vector.
%}
...
In Section \ref{sec:methods} we describe our methods for treating the mapmaking equation and our numerical experiments.
In Section \ref{sec:results} we present our results. 
In Section \ref{sec:discussion} we interpret the mapmaking approach.
In Section \ref{sec:conclusions} we conclude.
In appendices we derive the prescription for our cooling schedule.
...
\citet{2018A&A...620A..59P} showed that the conjugate gradient method can be easily applied to the cooled map-making problem.  In our notation, this is equivalent to iterating on
\begin{align}
  \qty(\Pdagger \inv{N(\eta_i)} P)\, \hatm(\eta_i) = \Pdagger \inv{N(\eta_i)} \vbd,
\label{map making eq eta}
\end{align}
%\begin{align}
%\Pdagger \inv{N(\eta)}  P\ \hatm(\eta) = \Pdagger \inv{N(\eta)} \vbd
%\label{map making eq eta}
%\end{align}
as we adjust the parameter through a set of levels $\{\eta_i\}$, which may change every iteration or less frequently. 
\textcolor{red}{
...
In our numerical experiments, we confirm that the conjugate gradient approach is converging faster than the fixed point iterations suggested by the messenger mapmaking method in \citet{Huffenberger_2018}.  For simplicity we fix the preconditioner to $M= \Pdagger P$ for all of calculations.
...
$\hatm_0 = \inv{\qty(\Pdagger P)} \Pdagger \vbd$,
...
Therefore, the cooling scheme requires the inverse cooling parameter $\eta$ increase as $0 = \eta_0 \leq \eta_1 \leq \cdots \leq \eta_{\rm final} = 1$, at which point we arrive at the desired mapmaking equation,
and $\hatm = \hatm(1)$.
...
\end{align}
is smaller than a tenth of the pixel variance given by the white noise level of the noise model,
\begin{align}
\norm{\vb{r}(\eta_i)}  < 0.1 {\sigma^2_{\rm pix}}.
...
\label{etai rule}
\end{align}
where $\bar N_f$ is the frequency representation of the non-uniform part of the covariance.
...
we change $\inv{N}$ to $\inv{N(\eta)}$, and it won't add extra cost,
...
over vanilla conjugate gradient method, which also combine the left graph in Figure~(\ref{apo noise chi2})),
...
%We start with initial map $\vbm_0 = \hatm(0)$, and the initial $\chi^2$ value is $\chi^2(\vbm_0) = \chi^2(\hatm(0),0)$.
%Our ultimate goal is to find $\hatm(1)$ which gives minimum $\chi^2$ value $\chi^2(\hatm) = \chi^2(\hatm(1),1)$.
%The difference between minimum and initial $\chi^2$ value
%\begin{align}
%\chi^2(\hatm(1),1) - \chi^2(\hatm(0),0) = \int_0^1 \dd{\eta}
%\dv{\eta} \chi^2(\hatm(\eta),\eta)
%\end{align}
%is the same for different $\eta$ series.
%Since $N$ is diagonal in frequency space,
%and using Eq.~(\ref{d chi2}) we could write the integrand $ \dv{\eta} \chi^2(\hatm(\eta),\eta) $
%as sum of all frequency modes with weight $\inv{N_f(\eta)} \Nbar_f \inv{N_f(\eta)}$ such as
%\begin{align}
%\dv{\eta} \chi^2(\hatm(\eta),\eta) 
%= - \sum_f \qty|\vbd - P \hatm(\eta)|_f^2 \ \frac{\Nbar_f}{\qty(\tau + \eta \Nbar_f)^2}
%\label{dchi2 freq}
%\end{align}
%where subscript $f$ means the covariance under frequency representation.
%When $\eta = 0$, the weight $\Nbar_f/\tau^2$ is large for high noise frequency mode (large $\Nbar_f$).
%As $\eta$ increases the weight for high noise mode is decreasing.
%the Eq.~(\ref{dchi2 freq}) would have large contribution from these mode.


%$\chi^2$ could be written as a sum of all frequency mode with weight $\inv{N}_f$, such as
%$\chi^2(\vbm) = \sum_f \qty|(\vbd-P\vbm)_f|^2 \inv{N}_f$.
%Which means $\chi^2(\vbm)$ would favor the low noise mode over high 
%noise ones.
...
%$\chi^2(\vbm,\eta)$ defined in Eq.~(\ref{chi2 eta formula}).
%For $\eta=0$, $N^{-1}(0) \propto I$ and the estimated map $\hatm(0)$
...
%frequency equally.
%Then as $\eta$ slowly increases, it gradually shifts focus from the highest noise to the lowest noise part.


%Notice that when $\eta$ is very small, 
...
For $\eta=0$, $N^{-1}(0) \propto I$ and the estimated map $\hatm(0)$
does not prioritize any frequency mode.
As we slowly increase $\eta$, we decrease the weight for the high noise frequency modes,
